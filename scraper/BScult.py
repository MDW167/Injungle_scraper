...
def getProducts(link, productList):
    ...
    parent      = soup.find_all('div', { 'class' : 'productGridItem js-product-grid-item' }) # find div der indeholder alle sidens produkter

    for x in parent: 
        productList += list(map(lambda n : base + n['href'].split('#')[0], x.find_all('a', href = True))) # generer links til alle sidens produkter i en liste
...
while ( load_number <= 30 ):
    products = getProducts('https://www.cultbeauty.co.uk/xhr/partial/products?loadedAmount=' + str(load_number) + '&path=%2Fskin-care.html&parameters=%7B%7D', productList)  #  /products?loadedAmount=1980  # https://www.cultbeauty.co.uk/xhr/partial/products?loadedAmount=1980&path=%2Fskin-care.html&parameters=%7B%7D
    load_number += 30
